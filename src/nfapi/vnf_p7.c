...
{	
	...
	else
	{
		...
		else
		{
			...
			// ORAN_OAI_procSlotInd(&ind); // MWNL TEST
...
void vnf_handle_nr_rx_data_indication(void *pRecvMsg, int recvMsgLen, vnf_p7_t* vnf_p7)
{
	...
	else
	{
		...
		else
		{
			...
			// ORAN_OAI_procRxDataInd(&ind); // [MWNL]
...
{
	uint8_t *recvMsg = pRecvMsg; // MWNL TEST
	...
	// printf("[MWNL]  ================= 0x%4x  [MWNL]\n", messageHeader.checksum);
	// for(int i = 0; i < recvMsgLen; i++)
	// 	printf("%d  ", recvMsg[i]);
	// printf("\n");
