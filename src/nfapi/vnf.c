...
{
    printf("\n[NFAPI VNF]  ->  Adding phy p5_idx:%d phy_id:%d", info->p5_idx, info->phy_id);
...
{
    //NFAPI_TRACE(NFAPI_TRACE_DEBUG, "config->pnf_list:%p", config->pnf_list);
    ...
    while (curr != 0)
    {
        ...
        {
            //NFAPI_TRACE(NFAPI_TRACE_ERROR, "%s : curr->p5_idx:%d p5_idx:%d", __FUNCTION__, curr->p5_idx, p5_idx);
            return curr;
        }

        //NFAPI_TRACE(NFAPI_TRACE_ERROR, "%s : curr->next:%p", __FUNCTION__, curr->next);
...
void vnf_nr_handle_pnf_param_response(void* pRecvMsg, int recvMsgLen, nfapi_vnf_config_t* config, int p5_idx)
{
    ...
    {
        //NFAPI_TRACE(NFAPI_TRACE_ERROR, "%s : NULL parameters", __FUNCTION__);
    }
    else
    {
        printf("\n[NFAPI VNF]  ->  Received PNF_PARAM.reponse");
        ...
        {
            //NFAPI_TRACE(NFAPI_TRACE_ERROR, "%s: Unpack message failed, ignoring", __FUNCTION__);
        }
...
void vnf_nr_handle_pnf_config_response(void* pRecvMsg, int recvMsgLen, nfapi_vnf_config_t* config, int p5_idx)
{
    ...
    {
        printf("\n[NFAPI VNF]  ->  Received PNF_CONFIG_RESPONSE");
...
void vnf_nr_handle_pnf_start_response(void* pRecvMsg, int recvMsgLen, nfapi_vnf_config_t* config, int p5_idx)
{
    ...
    {
        printf("\n[NFAPI VNF]  ->  Received PNF_START_RESPONSE");
...
void vnf_handle_pnf_stop_response(void* pRecvMsg, int recvMsgLen, nfapi_vnf_config_t* config, int p5_idx)
{
    ...
    {
        printf("\n[NFAPI VNF]  ->  Received PNF_STOP_RESPONSE");
...
void vnf_handle_start_response(void* pRecvMsg, int recvMsgLen, nfapi_vnf_config_t* config, int p5_idx)
{
    ...
    {
        printf("\n[NFAPI VNF]  ->  Received START_RESPONSE");
...
void vnf_nr_handle_start_response(void* pRecvMsg, int recvMsgLen, nfapi_vnf_config_t* config, int p5_idx)
{
    ...
    {
        printf("\n[NFAPI VNF]  ->  NR Received START_RESPONSE");
...
void vnf_handle_stop_response(void* pRecvMsg, int recvMsgLen, nfapi_vnf_config_t* config, int p5_idx)
{
    ...
    {
        printf("\n[NFAPI VNF]  ->  Received STOP.response");
...
void vnf_handle_measurement_response(void* pRecvMsg, int recvMsgLen, nfapi_vnf_config_t* config, int p5_idx)
{
    ...
    {
        printf("\n[NFAPI VNF]  ->  Received MEASUREMENT.response");
...
void vnf_handle_rssi_response(void* pRecvMsg, int recvMsgLen, nfapi_vnf_config_t* config, int p5_idx)
{
    ...
    {
        printf("\n[NFAPI VNF]  ->  Received RSSI.response");
...
void vnf_handle_rssi_indication(void* pRecvMsg, int recvMsgLen, nfapi_vnf_config_t* config, int p5_idx)
{
    ...
    {
        printf("\n[NFAPI VNF]  ->  Received RSSI.indication");
...
void vnf_handle_cell_search_response(void* pRecvMsg, int recvMsgLen, nfapi_vnf_config_t* config, int p5_idx)
{
    ...
    {
        printf("\n[NFAPI VNF]  ->  Received CELL_SEARCH.response");
...
void vnf_handle_cell_search_indication(void* pRecvMsg, int recvMsgLen, nfapi_vnf_config_t* config, int p5_idx)
{
    ...
    {
        printf("\n[NFAPI VNF]  ->  Received CELL_SEARCH.indication");
...
void vnf_handle_broadcast_detect_response(void* pRecvMsg, int recvMsgLen, nfapi_vnf_config_t* config, int p5_idx)
{
    ...
    {
        printf("\n[NFAPI VNF]  ->  Received BROADCAST_DETECT.response");
...
void vnf_handle_broadcast_detect_indication(void* pRecvMsg, int recvMsgLen, nfapi_vnf_config_t* config, int p5_idx)
{
    ...
    {
        printf("\n[NFAPI VNF]  ->  Received BROADCAST_DETECT.indication");
...
void vnf_handle_system_information_schedule_response(void* pRecvMsg, int recvMsgLen, nfapi_vnf_config_t* config, int p5_idx)
{
    ...
    {
        printf("\n[NFAPI VNF]  ->  Received SYSTEM_INFORMATION_SCHEDULE.response");
...
void vnf_handle_system_information_schedule_indication(void* pRecvMsg, int recvMsgLen, nfapi_vnf_config_t* config, int p5_idx)
{
    ...
    {
        printf("\n[NFAPI VNF]  ->  Received SYSTEM_INFORMATION_SCHEDULE.indication");
...
void vnf_handle_system_information_response(void* pRecvMsg, int recvMsgLen, nfapi_vnf_config_t* config, int p5_idx)
{
    ...
    {
        printf("\n[NFAPI VNF]  ->  Received SYSTEM_INFORMATION.response");
...
void vnf_handle_system_information_indication(void* pRecvMsg, int recvMsgLen, nfapi_vnf_config_t* config, int p5_idx)
{
    ...
    {
        printf("\n[NFAPI VNF]  ->  Received SYSTEM_INFORMATION.indication");
...
void vnf_handle_nmm_stop_response(void* pRecvMsg, int recvMsgLen, nfapi_vnf_config_t* config, int p5_idx)
{
    ...
    {
        printf("\n[NFAPI VNF]  ->  Received NMM_STOP.response");
...
void vnf_handle_vendor_extension(void* pRecvMsg, int recvMsgLen, nfapi_vnf_config_t* config, int p5_idx, uint16_t message_id)
{
    ...
    if (config->allocate_p4_p5_vendor_ext && config->deallocate_p4_p5_vendor_ext)
    {
        ...
        {
            printf("\n[NFAPI VNF]  ->  failed to allocate vendor extention structure");
        }
    }
}
...
void vnf_nr_handle_p4_p5_message(void* pRecvMsg, int recvMsgLen, int p5_idx, nfapi_vnf_config_t* config)
{
    ...
    default:
    {
        ...
        {
            //NFAPI_TRACE(NFAPI_TRACE_ERROR, "%s P5 Unknown message ID %d", __FUNCTION__, messageHeader.message_id);
        }
    }
    break;
    }
...
int vnf_nr_read_dispatch_message(nfapi_vnf_config_t* config, nfapi_vnf_pnf_info_t* pnf)
{
    ...
    if (1)
    {
...

        {
            ...
            if (message_size == -1)
            {
                printf("\n[NFAPI VNF]  ->  VNF Failed to peek sctp message size errno:%d", errno);
                return 0;
            }
            ...
            if (unpack_result < 0)
            {
                printf("\n[NFAPI VNF]  ->  VNF Failed to decode message header %d", unpack_result);
                return 0;
            }
        ...
        if (message_size > stack_buffer_size)
        {
            ...
            if (dynamic_buffer == NULL)
            {
                ...
                printf("\n[NFAPI VNF]  ->  VNF Failed to allocate dynamic buffer for sctp_recvmsg size:%d", message_size);
                return -1;
            }
...

        {
            ...
            if (recvmsg_result == -1)
            {
                printf("\n[NFAPI VNF]  ->  Failed to read sctp message size errno:%d", errno);
            }
            else
            {
                if (flags & MSG_NOTIFICATION)
                {
                    printf("\n[NFAPI VNF]  ->  Notification received from %s:%u", inet_ntoa(addr.sin_addr), ntohs(addr.sin_port));
                    // todo - handle the events
                }
                else
                {
                    printf("\n[NFAPI VNF]  ->  Received message fd:%d from %s:%u assoc:%d on stream %d, PPID %d, length %d, flags 0x%x",
                            pnf->p5_sock,
                    ...
                    {
                        //NFAPI_TRACE(NFAPI_TRACE_WARN, "sctp_recvmsg: unhandled mode with flags 0x%x", flags);
...
int
vnf_read_dispatch_message(nfapi_vnf_config_t* config, nfapi_vnf_pnf_info_t* pnf)
{
    if (1)
    {
        int socket_connected = 1;
...
static int
vnf_send_p5_msg(nfapi_vnf_pnf_info_t* pnf, const void* msg, int len, uint8_t stream)
{
    // printf("\n MESSAGE SENT: ");
    // for(int i=0; i<len; i++){
    ...
    // //NFAPI_TRACE(NFAPI_TRACE_ERROR, "%s len:%d stream:%d", __FUNCTION__, len, stream);
    ...
    if (result != len)
    {
        ...
        {
            // error
            // //NFAPI_TRACE(NFAPI_TRACE_ERROR, "sctp sendto failed errno: %d", errno);
            DU_LOG("\nERROR  -->  vnf : SCTP sento failed errno. : %d", errno);
        }
        else
        {
...
int vnf_nr_pack_and_send_p5_message(vnf_t* vnf, uint16_t p5_idx, nfapi_p4_p5_message_header_t* msg, uint16_t msg_len)
{
    ...
    if (pnf)
    {
        ...
        {
            // //NFAPI_TRACE(NFAPI_TRACE_ERROR, "nfapi_p5_message_pack failed with return %d", packedMessageLength);
            return -1;
        }
    ...
    {
        // printf("\n[NFAPI VNF]  ->  %s() cannot find pnf info for p5_idx:%d", __FUNCTION__, p5_idx);
        return -1;
    }
}
...
int
vnf_pack_and_send_p4_message(vnf_t* vnf, uint16_t p5_idx, nfapi_p4_p5_message_header_t* msg, uint16_t msg_len)
{
    nfapi_vnf_pnf_info_t* pnf = nfapi_vnf_pnf_list_find(&(vnf->_public), p5_idx);
    ...
    if (pnf)
    {
        ...
        if (packedMessageLength < 0)
        {
            //NFAPI_TRACE(NFAPI_TRACE_ERROR, "nfapi_p4_message_pack failed with return %d", packedMessageLength);
            return -1;
        }
