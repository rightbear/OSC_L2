...
{	
	...
	{	
		...
		if (slot_ind) {
			...
			DU_LOG("[MWNL]  ->  The slot_ind sfn : %d, slot : %d\n", slot_ind->sfn, slot_ind->slot);
			...
			if (setup_time > 3 && prev_slot != UL_INFO.slot) { //Give the VNF sufficient time to setup before starting scheduling  && prev_slot != gNB->UL_INFO.slot
	...
	printf("[NFAPI P7] ->  Closing p7 socket\n");
	close(vnf_p7->socket);

	printf("[NFAPI P7] ->  %s() returning\n", __FUNCTION__);
...
{
	printf("%s(config:%p phy_id:%d pnf_addr:%s pnf_p7_port:%d)\n", __FUNCTION__, config, phy_id,  pnf_p7_addr, pnf_p7_port);
